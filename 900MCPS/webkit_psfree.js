...
function check_spray(views) {
    if (views.length !== num_reuse) {
        st.innerHTML(`views.length: ${views.length}`);
...
async function setup_ar(save) {
    ...
    // st.innerHTML(`total_sleep: ${total_sleep}`);
    ...
    while (true) {
        ...
        for (let i = 0; i < str_arr.length; i++) {
            if (str_arr[i].length > 0xff) {
                ...
                // st.innerHTML(`str len: ${rstr.length}";
                // st.innerHTML(view);
                // st.innerHTML(`read address: ${read64(view, strimpl_m_data)}";
                break;
            }
        }
        if (!found) {
        ...
        // st.innerHTML(`num_spray: ${num_spray}`);
        return;
    }
}
